...
BEGIN
    ...
    START TRANSACTION;
        ...
        IF ROW_COUNT() = 0 THEN
            set error_message = CONCAT('No matching record found in stars for id: ', p_star_id);
            -- Trigger custom error, invoke EXIT HANDLER
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_message;
        END IF;
...
BEGIN
    ...
    START TRANSACTION;
        ...
        -- ROW_COUNT() returns the number of rows affected by the preceding statement.
        IF ROW_COUNT() = 0 THEN
            set error_message = CONCAT('No matching record found in show for id: ', p_show_id);
            -- Trigger custom error, invoke EXIT HANDLER
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_message;
        END IF;
...
-- DELETE Show_Customers

DROP PROCEDURE IF EXISTS sp_DeleteShow_Customers;

DELIMITER //

CREATE PROCEDURE sp_DeleteShow_Customers(
    IN p_show_id INT, 
    IN p_customer_id INT)
BEGIN
    DECLARE error_message VARCHAR(255); 
    ...
    START TRANSACTION;
        DELETE FROM Show_Customers WHERE show_id = p_show_id AND customer_id = p_customer_id;

        -- ROW_COUNT() returns the number of rows affected by the preceding statement.
        IF ROW_COUNT() = 0 THEN
            set error_message = CONCAT('No matching record found in show_customers for id: ', p_customer_id);
            -- Trigger custom error, invoke EXIT HANDLER
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_message;
        END IF;
